...
@php
    ...
    if (!empty($preview)) {
        if (filter_var($preview, FILTER_VALIDATE_URL)) {
            ...
            $defaultPreview = asset('storage/' . $preview);
        }
    } else {
        $defaultPreview = '';
    }
...
<div {{ $attributes->only('wire:key')->merge($styleAttributes->getAttributes())->class($classes) }} data-flux-input-file
    wire:ignore tabindex="0" x-data="{
    ...
    <input x-ref="input" x-on:click.stop x-init="Object.defineProperty($el, 'value', {
        ...
        class="sr-only" tabindex="-1" {{ $attributes->except('wire:key') }} {{ $multiple ? 'multiple' : '' }}
